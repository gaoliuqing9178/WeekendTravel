# Contract: INT-002 Family scenario complete path

## 本轮目标

- 跑通真实 real mode 家庭场景完整链路：
  `POST /api/plan` -> `GET /api/plan/{planId}/stream` -> `plan_ready` -> `POST /api/plan/{planId}/execute` -> `execute_result` -> `done`。
- 验证前端使用真实后端，而不是 mock fixture，展示 family plan、确认执行、执行结果和最终完成态。
- 只覆盖 family happy path；friends 完整路径留给 `INT-003`。

## 明确不做

- 不接真实支付、真实订座、真实配送、真实地图或真实第三方服务。
- 不扩展 OpenAI / LLM 接入；规划决策仍由后端确定性状态机和本地 Mock Tool 完成。
- 不修改 `docs/api-contract.md` 既有字段；本轮只实现其中已经声明的 execute / execute_result / done 链路。
- 不把 bookingFail、restaurantFull、routeTooFar 等异常链路作为本轮 happy path 的放行条件。

## 用户路径

1. 前端以 `VITE_API_MODE=real` 启动。
2. 用户保留或输入家庭 Demo 文本，场景为 `family`。
3. 前端调用 `POST /api/plan`，后端返回非空 `planId` 和 `status=processing`。
4. 前端连接 `GET /api/plan/{planId}/stream`。
5. 后端推送规划链路事件，至少包含 `heartbeat`、`state_change`、`tool_call`、`tool_result`、`plan_ready`。
6. 前端渲染方案卡，Agent 到达 `CONFIRM`。
...
- `execute_result`
  ...
  - family happy path 至少覆盖 `reserve_table` 和 `send_message` 两类动作。
...
- evaluator 需要记录：
  ...
  - 页面最终可见 `DONE`、`execute_result`、mock 确认号或执行完成计数。

## 失败阈值

- family real mode 不能到达 `plan_ready`，失败。
...
- 没有独立 evaluator 证据，不得把 `INT-002` 标记为 `verified`。
